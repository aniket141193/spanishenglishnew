...
public class OperatorController {
	...
	@RequestMapping(value = "/machinecollectionlist")
	public String machinecollectionlist(
			@RequestParam("establishmentCollectionId") String establishmentCollectionIdStr,
			HttpServletRequest request, HttpServletResponse response,
			ModelMap model) {

		EstablishmentCollection estaColl = newCollectionServices
				.getEstablishmentCollectionById(Long
						.parseLong(establishmentCollectionIdStr));
	...
	public String newcollection(
		...
		for (Machine machine : establishment.getMachine()) {
			TempMachine tempMachine = new TempMachine(machine, loggedOperator,
					establishment, false);
			tempMachineServices.saveTempMachine(tempMachine);
		}

		EstablishmentCollection estColl = new EstablishmentCollection();
		estColl.setAdmin(loggedOperator.getAdmin());
		estColl.setOperator(loggedOperator);
		estColl.setEstablishment(establishment);
		newCollectionServices.addOrUpdateEstablishmentCollection(estColl);
		estColl = newCollectionServices.getLastEstablishmentCollection();
		model.addAttribute("establishmentId", establishmentIdStr);
		model.addAttribute("establishmentCollectionId", estColl.getId());
	...
	public String machinecollection(HttpServletRequest request,
		...
		String collectionDateString = request.getParameter("collectionDate");
		DateFormat df = new SimpleDateFormat("MM/dd/yyyy");
		Date collectionDate = null;
		try {

			collectionDate = df.parse(collectionDateString);
			estaColl.setCollectionDate(collectionDate);
	...
	@RequestMapping(value = "/machineCollectionCalculate")
	public String machineCollectionCalculate(HttpServletRequest request,
			HttpServletResponse response, ModelMap model) {
		...
		double fails = Double.valueOf(request.getParameter("fail"));
		double realCollection = Double.valueOf(request
				.getParameter("realCollection"));
		long inputCounterCurrent = Long.valueOf(request
				.getParameter("inputCounterCurrent"));
		long outputCounterCurrent = Long.valueOf(request
				.getParameter("outputCounterCurrent"));

		// calculate theoreticalCollection
		long input = inputCounterCurrent - machine.getMachineInput();
		...
		mcn.setStartDate(startDate);
		mcn.setEndDate(endDate);

		mcn.setFails(fails);

		mcn.setInputCounterCurrent(inputCounterCurrent);
		mcn.setOutputCounterCurrent(outputCounterCurrent);

		mcn.setInputCounterLast(machine.getMachineInput());
		mcn.setOutputCounterLast(machine.getMachineOutput());

		mcn.setDiffInCash(diffInCash);
		mcn.setDiffInPercentage(diffInPercentage);

		mcn.setNetTotal(netTotal);
		mcn.setTheoreticalCollection(theoreticalCollection);
		mcn.setRealCollection(realCollection);

		mcn.setMachine(machine);
		mcn.setEstablishmentCollection(estaColl);

		newCollectionServices.addOrUpdateMachineCollectionNew(mcn);
		...
		Set<MachineCollectionNew> mcnList = estaColl.getMachineCollectionNew();
		if (mcnList == null)
			mcnList = new HashSet<MachineCollectionNew>();

		mcnList.add(mcn);
		estaColl.setMachineCollectionNew(mcnList);
		newCollectionServices.addOrUpdateEstablishmentCollection(estaColl);

		// done tempMachine
		TempMachine tempMachine = tempMachineServices
				.getTempMachineByMachine(machine);
		tempMachine.setDone(true);
		tempMachine.setNetTotal(netTotal);
		tempMachineServices.saveTempMachine(tempMachine);
		...
		for (TempMachine tm : tempMachineList) {
			if (!tm.isDone()) {
				machine = tm.getMachine();
				isDone = false;
				break;
			}
		}
	...
	@RequestMapping(value = "/establishmentCollectionCalculate")
	public String establishmentCollectionCalculate(HttpServletRequest request,
			HttpServletResponse response, ModelMap model) {
		...
		double otherExpenses = Double.valueOf(request
				.getParameter("otherExpenses"));
		double playerGift = Double.valueOf(request.getParameter("playerGift"));

		estaColl.setPlayersGift(playerGift);
		estaColl.setOtherExpenses(otherExpenses);

		OtherExpenses otherExpense = percentageDistributionServices
				.getOtherExpenses(establishment.getAdmin().getId(),
		...
		double estAgreedPercentages = 0;
		Set<MachineCollectionNew> mcColls = estaColl.getMachineCollectionNew();
		for (MachineCollectionNew machineCollectionNew : mcColls) {
			Set<AgreedPercentage> agreedPercentageList = percentageDistributionServices
					.getAgreedPercentageByMachine(machineCollectionNew
							.getMachine().getId());
